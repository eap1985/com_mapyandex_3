...
class MapYandexLoader extends JLoader
{
	...
	public static function import($filePath, $base = null, $key = 'libraries.') {
		...
		if (!isset($paths[$keyPath])) {
			if ( !$base ) {
				$base =  JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mapyandex'.DS.'libraries';
			}
			...
			if (strpos($filePath, 'mapyandex') === 0) {

				$className	= 'mapyandex'.$className;
			...
			MapYandexLoader::$paths[$keyPath] = $rs;
		}
		
		return MapYandexLoader::$paths[$keyPath];
	}
}

function mapyandeximport($path) {
	
	return MapYandexLoader::import($path);
}
