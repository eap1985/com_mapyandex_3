...
*/
	function getAllRoute()
	{
		...
		if (!$this->_allroute)
		{
			...
			." WHERE ".$db->quoteName('map_type')." = 'map'";
			$db->setQuery($query);
...
*/
	function getFoobar()
	{
		...
		if (!$this->_foobar)
		{
			$db = $this->getDBO();
			$query = "SELECT * FROM ".$db->quoteName('#__map_yandex') 
			." WHERE ".$db->quoteName('id')." = ".$this->_id." AND ".$db->quoteName('map_type')." = 'map'";
			$db->setQuery($query);
	...
	{	
		...
		// Store the web link table to the database
			$db = $this->getDBO();
			$query = "UPDATE #__map_yandex set ".$db->quoteName('region_map_yandex')." = '".$this->textnl($el)
			."', ".$db->quoteName('map_region_style')." = '".$style."' WHERE ".$db->quoteName('id')." = ".$data['id'];
	...
	function hit()
	{
		...
		$db = JFactory::getDBO();
		$db->setQuery('UPDATE #__map_yandex SET '.$db->quoteName('hits').' = '.$db->quoteName('hits').' + 1 '.'WHERE id = '.$this->_id);
		$db->query();
	}
	/**
	 * 
